...
# Path: main.py
def create_files(
    app_name: str = "sampleApp", project_name: str = "sampleProject"
) -> None:
    ...
    with open(os.path.join(path_forURLsPY_project, "urls.py"), "r+") as fp:
        ...
        for i in list:
            ...
            if "urlpatterns = [" in i:
                list.insert(
                    list.index(i) + 1, f"    path('', include('{app_name}.urls')),"
                )
...
def quickstart_django(
    project_name: str = "sampleProject", app_name: str = "sampleApplication"
) -> None:

    required_packages = ["django"]
    essential_dirs = ["templates", "static", "media"]

    install_requirements(required_packages[0])
    create_project(project_name=project_name)
    create_app(app_name=app_name)
    create_directories(essential_dirs=essential_dirs)
    create_files(project_name=project_name, app_name=app_name)

    print(
        f"""
{"*"*100}
Your Django app is ready to use..!
...
if __name__ == "__main__":
    quickstart_django("custom proj", "customApp")
